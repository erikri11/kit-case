...
export function validateCreate(body: unknown): string | null {
  ...
  const { name, image, category, type, quantity, currency, price, sku, status } = data;

  if (!name || typeof name !== "string" || !name.trim()) return "Name is required";
  if (image !== undefined && image !== null && typeof image !== "string") return "Image must be a string or null";
  ...
  if (sku !== undefined && typeof sku !== "string") return "SKU must be a string";
